...
if __name__ == "__main__":
    ...
    for i, case in enumerate(cases):
        ...
        with open('results-uni.txt', 'a') as output:
        ...
        np.savetxt('histogram/histogram-'+case+'-uni.dat', np.vstack((np.insert(bin_edges, 0, result.mean()), np.insert(np.append(hist,0), 0, result.std()))).T)
    # os.chdir('histogram')
