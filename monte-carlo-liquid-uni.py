...
def run_case(n, fuel, P0, T0, PC, mfuel, Ta, mix):
    ...
    # Ta_dist = uniform(loc=Ta-sigma_Ta, scale=sigma_Ta*2)
    ...
    T0_dist = uniform(loc=T0-sigma_T0, scale=sigma_T0*2)
...
if __name__ == "__main__":
    ...
    for i, case in enumerate(cases):
        ...
        with open('results-liquid-uni.txt', 'a') as output:
        ...
        np.savetxt('histogram/histogram-'+case+'-liquid-uni-T0.dat', np.vstack((np.insert(bin_edges, 0, result.mean()), np.insert(np.append(hist,0), 0, result.std()))).T)
    # os.chdir('histogram')
