...
def run_case(n, fuel, P0, T0, PC, mfuel, Ta, mix):
    ...
    Ta_dist = triang(loc=Ta-sigma_Ta, scale=sigma_Ta*2, c=0.5)
    ...
    T0_dist = triang(loc=T0-sigma_T0, scale=sigma_T0*2, c=0.5)
...
if __name__ == "__main__":
    ...
    for i, case in enumerate(cases):
        ...
        with open('results-liquid-tri.txt', 'a') as output:
        ...
        np.savetxt('histogram/histogram-'+case+'-liquid-tri.dat', np.vstack((np.insert(bin_edges, 0, result.mean()), np.insert(np.append(hist,0), 0, result.std()))).T)
    # os.chdir('histogram')
